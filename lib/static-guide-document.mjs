...
const STATIC_SOURCE_UNAVAILABLE = "STATIC_SOURCE_UNAVAILABLE";
...
function staticSourceUnavailableResponse() {
  return Response.json(
    { ok: false, code: STATIC_SOURCE_UNAVAILABLE },
    {
      status: 503,
...
export async function fetchStaticSourceResponse(selfBinding, url, locale) {
  if (!selfBinding || typeof selfBinding.fetch !== "function") {
    return staticSourceUnavailableResponse();
  }

  const sourceRequest = new Request(staticGuideSourceUrl(url), {
    method: "GET",
    headers: staticSourceRequestHeaders(locale)
  });

  try {
    return await selfBinding.fetch(sourceRequest);
  } catch {
    return staticSourceUnavailableResponse();
  }
}
