...
import { LEGAL_RUNTIME_SOURCES } from "./legal-runtime-sources.generated.mjs";
...
export function loadApprovedLegalDocument(route) {
  ...
  const source = LEGAL_RUNTIME_SOURCES[route];
  if (typeof source !== "string") {
    throw new Error(`No approved legal runtime source is registered for ${route}.`);
  }
  return parseApprovedLegalSource(source, expected);
}
