  ...
  return (
    <div className="creator-site-page">
      ...
      <section className="section-block section-rule creator-site-final">
        <div className="site-container creator-site-final__layout"><div><h2>{copy.final.titleLines ? copy.final.titleLines.map((line) => <span key={line}>{line}</span>) : copy.final.title}</h2><p>{copy.final.body}</p></div><ActionLink href={copy.final.href}>{copy.final.action}</ActionLink></div>
      </section>
    </div>
  );
}
